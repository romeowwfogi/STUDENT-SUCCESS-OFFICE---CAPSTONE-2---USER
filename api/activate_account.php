...
try {
    ...
    $sql1 = "INSERT INTO tokenization (user_id, name, value, expires_at)
             VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 1 DAY))";
    $types1 = "iss";
    $params1 = [$ACCOUNT_ID, 'ACTIVATE_ACCOUNT', $token];
    ...
    $sql = "SELECT subject, html_code 
        FROM email_template 
        WHERE is_active = 1 
          AND title = 'Account Reactivation'";
    $result = executeSelect($conn, $sql);
    ...
    $MESSAGE = getConfigValue(
        $conn,
        'REACTIVATION_ACCOUNT_FAILED',
        "Something went wrong while activation your account. Please try again."
    );
    http_response_code(500);
    echo json_encode([
        "success" => false,
